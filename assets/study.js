...
(function () {
  ...
  function paint() {
    ...
    m.innerHTML =
      ...
      '<button class="btn ghost" id="ppmNext" type="button" onclick="ppmDrillNext()">다음</button>' +
    ...
    if (inp) {
      ...
      inp.onkeydown = function (ev) {
        if (ev.key === "Enter" && !ev.isComposing) { ev.preventDefault(); window.ppmDrillCheck(); }
      };
    }
  }
  ...
  window.ppmDrillNext = function () {
    if (!D || D.i >= D.items.length) return;
    D.i++;
