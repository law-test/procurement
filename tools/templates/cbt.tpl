<main id="main-content" tabindex="-1" class="wrap card-wrap">
  ...
  <div class="card-setup" id="setup">
    <div class="row"><label for="selKind">방식</label><select id="selKind">
      <option value="real">실전 · 필기 80문항 120분</option>
      <option value="subject">과목별 연습</option>
      <option value="practice">실기 실무 문항 연습</option>
    </select></div>
    <div class="row" id="rowSubject" hidden><label for="selSubject">과목</label><select id="selSubject"></select></div>
    <div class="row" id="rowCount" hidden><label for="selCount">문항</label><select id="selCount">
      <option value="10">10문항</option><option value="20" selected>20문항</option><option value="40">40문항</option>
    </select></div>
    <div class="row"><label for="selTime">시간</label><select id="selTime">
  ...
  <div id="exam" tabindex="-1" hidden>
    <div class="cbt-bar">
      <span id="clock" class="clock" role="timer" aria-label="남은 시간">--:--</span>
      <span id="counter" role="status"></span>
...
<script>
var E = null, TMR = null, starting = false, startVersion = 0;
...
function cStart() {
  ...
  return Promise.all(plan.map(function (p) { return PPMQ.loadQuiz(p[0]); })).then(function (res) {
    ...
    if (kind === "real" && res.some(function (j, i) { return j.items.length < plan[i][1]; })) {
      $("stat").textContent = "실전 구성에 필요한 문항이 부족합니다. 과목별 연습을 선택해 주세요.";
      return;
    }
...
function cSubmit(automatic) {
  ...
  $("result").innerHTML =
    ...
    '</tbody></table></div>' +
    '<p class="lede">' + (E.kind !== "real" ? '선택한 범위의 연습 결과입니다. 틀린 문항을 확인해 보세요.'
      : fail ? '과목 하나가 40점 미만입니다. 그 과목부터 다시 보세요.'
      : rawAvg >= 60 ? '통상 합격선인 평균 60점 이상입니다.' : '평균이 60점에 못 미칩니다.') + '</p>' +
